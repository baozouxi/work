...
function change(v) {
    var str = "0";
    for (i = 0; i < v; i++) { str += ("_" + ($(s[i]).selectedIndex - 1));}
    ;
    var ss = $(s[v]);
    with (ss) {
        length = 0;
        ...
        if (v && $(s[v - 1]).selectedIndex > 0 || !v) {
            if (dsy.Exists(str)) {
                ...
                for (i = 0; i < ar.length; i++){
					options[length] = new Option(ar[i], i+1);
				}
...
function preselect(p_key) {
		...
		var province = parseInt(p_key.substr(0,2));
		var city = parseInt(p_key.substr(2,2));
		var town = parseInt(p_key.substr(4,2));
        $(s[0]).selectedIndex = province;
        change(1);
        $(s[1]).selectedIndex = city;
		change(2);
        $(s[2]).selectedIndex = town;
}
function show_add(o){
	var province = parseInt(o.substr(0,2))-1;
	var city = parseInt(o.substr(2,2))-1;
	var town = parseInt(o.substr(4,2))-1;
    var str = "0";
	var s_province = '';
	if(province>=0){
		if (dsy.Exists(str)){
			ar = dsy.Items[str];
			s_province = ar[province];
		}
	}
	var s_city = '';
	if(city>=0){
	...
	var s_town = '';
	if(town>=0){
		str = '0_'+province+'_'+city;
		if (dsy.Exists(str)){
			ar = dsy.Items[str];
			s_town = ar[town];
		}
	}
	document.write(s_province+s_city+s_town);
}
